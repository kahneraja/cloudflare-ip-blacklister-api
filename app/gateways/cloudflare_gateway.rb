class CloudflareGateway
  ...
  def self.get_accounts(key, email)
  ...
  def self.get_all_rules(key, email, account_id)
    page = 1
    response = get_rules(key, email, account_id, page)
    page = response['result_info']['page'].to_i
    total_pages = response['result_info']['total_pages'].to_i
    data = response['result']
    while (page < total_pages)
      page += 1
      page_response = get_rules(key, email, account_id, page)
  ...
  def self.get_rules(key, email, account_id, page=1)
    url = "https://api.cloudflare.com/client/v4/accounts/#{account_id}/firewall/access_rules/rules?configuration.target=ip&page=#{page}"
    puts url
    headers = auth_headers(key, email)
    response = HTTParty.get(url, headers: headers).response
    JSON.parse(response.body)
  ...
  def self.create_block_rule(key, email, account_id, body)
  ...
  def self.delete_rule(key, email, account_id, rule_id)
